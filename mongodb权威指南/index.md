...
MongoDB使用`find`方法进行查询。返回集合中文档的一个子集，子集的范围从0个文档到整个文档。要返回哪些文档由`find`的第一个参数决定，该参数是一个用于指定查条件的文档。
...
可通过`find`(或`findOne`)的第二个参数指定需要的返回的键。
...
`$lt`、`lte`、`$gt`、`$gte`、`$ne`都属于比较运算符，分别对应小于、小于等于、大于、大于等于、不等于，可以将它们组合使用以查找一个范围内的值。

```js
db.users.find({ age: { $gte: 18, $lte: 30 } });
```

`$in`可以用来查询一个键的多个值。`$nin`会返回与数组中所有条件都不匹配的文档。

```js
db.raffle.find({ ticket_no: { $in: [125, 365, 778] } });

db.raffle.find({ ticket_no: { $nin: [125, 365, 778] } });
```

`$in`能对单个键进行OR查询，而`$or`可以在多个键中查询任意的给定值。

```js
db.raffle.find({
  or: [{ ticket_no: { $in: [125, 365, 778] } }, { winner: true }],
});
```

OR类查询如果第一个参数能够匹配尽可能多的文档，则其**效率最高**。虽然总是可以用`$or`，但有可能就应该用`$in`，因为查询器可以更**高效**地对其进行处理。

`$not`是一个元条件运算符，可以用于任何其他条件之上。

取模运算符`$mod`会将查询的值除以第一个给定值，如果余数等于第二个给定值，则匹配成功。
...
游标可以理解为查询结果的迭代器(Iterator)，它并不是一次性把所有数据都返回给客户端，而是指向查询结果集中的一个“指针”，客户端可以通过它逐批获取数据。

MongoDB会使用游标返回`find`的执行结果。
...
```js
db.orders.find({userId: 10001, status: "SUCCESS"})
```
...
MongoDB索引设计经典原则：ESR(E = Equality, S = Sort, R = Range)。

### 第6章 特殊的索引和集合类型

地理空间索引、全文搜索索引、固定集合、TTL索引、使用GridFS存储文件

### 第7章 聚合框架

#### 7.2 阶段入门：常见操作

匹配(match)、投射(project)、排序(sort)、跳过(skip)和限制(limit)这5个阶段。

#### 7.3 $unwind

在聚合管道中处理数组字段时，通常需要包含一个或多个展开(unwind)阶段。这允许我们将指定数组字段中的每个元素都形成一个输出文档。

![图7-4 &dollar;unwind会从输入文档中获取一个数组，并为该数组中的每个元素创建一个输出文档](/images/202607/24/7-4.png "图7-4 &dollar;unwind会从输入文档中获取一个数组，并为该数组中的每个元素创建一个输出文档")

### 第8章 事务

什么时候使用事务，核心不是看“有几个操作”，而是看：

> 这些操作是否必须作为一个不可分割的业务事件，要么全部成功，要么全部失败。

MongoDB设计理念是：

> 优先通过合理的数据模型(嵌入文档)避免事务；只有跨文档一致性无法避免时才使用事务。
